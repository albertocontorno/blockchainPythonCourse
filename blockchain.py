...
#partecipants = set([owner])
    ...
    def load_data(self):
        try:
            with open("blockchain.json", mode="r") as f:
                ...
                self.__open_transactionsansactions = updated_transactions
    ...
    def save_data(self):
        try:
            with open("blockchain.json", mode="w") as f:
                ...
                f.write("\n")
                saveable_tx = [tx.__dict__.copy() for tx in self.__open_transactionsansactions]
    ...
    def proof_of_work(self):
        ...
        proof = 0
        while(not Verification.valid_proof(self.__open_transactionsansactions, last_hash, proof)):
            proof += 1
        return proof
    ...
    def get_balance(self):
        ...
        open_tx_sender = [tx.amount for tx in self.__open_transactionsansactions if tx.sender == partecipant]
    ...
    def add_transaction(self, recipient, sender, amount=1.0):
        ...
        if(Verification.verify_transaction(transaction, self.get_balance)):
            self.__open_transactionsansactions.append(transaction)
    ...
    def mine_block(self):
        ...
        copied_transactions = self.__open_transactionsansactions[:]
        copied_transactions.append(reward_transaction)
        block = Block(hashed_block, len(self.__chain),  copied_transactions, proof)
        self.__chain.append(block)
        self.__open_transactionsansactions = []
        self.save_data()
        return True
